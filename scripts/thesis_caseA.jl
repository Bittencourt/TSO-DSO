...
println(
    "  $(length(load_buses)) aggregator buses, T=$T, seed=$SEED, MEM price λ₀ (Fig 4.5)",
)
println("=" ^ 72)
...
ctx_dadp, welfare_dadp, _ =
    solve_welfare(FEEDER, PF, aggs; T = T, λ₀ = λ₀, allow_export = true, τ = 1e-2)
...
println(
    "  prosumer surplus = $(round(acct.prosumer; digits=4))   DSO surplus = $(round(acct.dso; digits=4))",
)
println("  SOCP exactness   = $(round(maxgap; sigdigits=2)) (certified exact)")
println(
    "  peak DADP        = $(round(maximum(dadp); digits=3))  vs  λ₀ peak = $(round(maximum(λ₀); digits=3))",
)
...
let
    ...
    axB = Axis(
        fig[2, 2];
        xlabel = "hour of day",
        ylabel = "charge(−) / discharge(+) (per-unit)",
        title = "(d) PV-battery",
        titlealign = :left,
    )
    ...
    axSoc = Axis(
        fig[2, 2];
        ylabel = "state of charge (per-unit)",
        yaxisposition = :right,
        ylabelcolor = :purple,
        yticklabelcolor = :purple,
    )
    hidespines!(axSoc)
    hidexdecorations!(axSoc)
    linkxaxes!(axB, axSoc)
...
let
    ...
    fig[0, 1] = Label(
        fig,
        "Thesis Case A — DLMP four-way decomposition (Fig 4.7) — bus-averaged";
        fontsize = 15,
        font = :bold,
    )
    ax = Axis(
...
let
    ...
    fig[0, 1] = Label(
        fig,
        "Thesis Case A — voltage profile |V| at aggregator buses (Fig 4.4)";
        fontsize = 15,
        font = :bold,
    )
    ax = Axis(
...
let
    ...
    fig[0, 1] = Label(
        fig,
        "Thesis Case A — DADP vs FIT: welfare & surplus split (page 98)";
        fontsize = 15,
        font = :bold,
    )
    ...
    barplot!(axW, [1, 2], [welfare_fit, welfare_dadp]; color = [:indianred, :seagreen])
    text!(
        axW,
        2,
        welfare_dadp;
...
let
    ...
    fig[0, 1] = Label(
        fig,
        "Thesis Case A — TSO↔DSO frontier exchange p₀ (DADP vs FIT)";
        fontsize = 15,
        font = :bold,
    )
    ax = Axis(
...
let
    ...
    fig[0, 1] = Label(
        fig,
        "Thesis Case A — DADP surface (aggregator buses × hour)";
        fontsize = 15,
        font = :bold,
    )
    ax = Axis(
...
println("\n" * "=" ^ 72)
@printf(
    "RESULT: DADP/FIT social-welfare ratio = %.4f  (thesis Case A target ≈ 1.25)\n",
    ratio
)
if ratio < 1.02
    println(
...
""",
    ...
    println(
        "DADP beats FIT by +$(round(100 * (ratio - 1); digits = 1))% social welfare (thesis Case A ≈ +25%).",
    )
end
