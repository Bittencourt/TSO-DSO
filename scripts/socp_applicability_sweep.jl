...
function highpv_house(bus; pv_scale, load_scale)
    ...
    return Aggregator(
        ...
        [Thermostatic(bus, 0.2, 0.05, 15.0, 30.0, 22.0, 0.0, 1.0, 0.5, TEMP),
            ...
            PVBattery(bus, 0.95, 1.0, 0.1, 0.0, 0.2, 0.1, BATT_λ...,
...
function ieee123_house(bus; pv_scale, load_scale, dev_scale)
    ...
    return Aggregator(
        ...
        [Thermostatic(bus, 0.2, 0.05, 15.0, 30.0, 22.0, 0.0, 1.0 * dev_scale, 0.5, TEMP),
            ...
            PVBattery(bus, 0.95, 1.0, 0.5 * load_scale, 0.0, 2.0 * load_scale,
                1.0 * load_scale, BATT_λ...,
                Float64[pv_scale * p for p in prof.pv])],
        Float64[load_scale * d for d in prof.demand],
    )
end
...
function sweep(substrate::Symbol; optimizer = nothing)
    if substrate === :highpv
        ...
        [0.10, 0.15, 0.20, 0.30, 0.40], [1.05, 1.075, 1.10]
        base = nothing
        mk = (vmax, ps, ls) -> (highpv_feeder(; vmax),
            [highpv_house(b; pv_scale = ps, load_scale = ls) for b in 2:3])
    else
        pvs, lds, vms = [0.4, 0.7, 1.0, 1.3, 1.7, 2.2], [0.95, 1.00, 1.05],
        [1.05, 1.075, 1.10]
        base = ieee123_modified()
        nodes = ieee123_load_nodes()
        mk = (vmax, pm, lm) -> (with_vmax(base, vmax),
            [ieee123_house(b; pv_scale = IEEE123_BASE.pv * pm,
                load_scale = IEEE123_BASE.load * lm, dev_scale = IEEE123_BASE.dev)
             for b in nodes])
    end
    ...
    for vmax in vms, ls in lds, ps in pvs
        ...
        row = try
            ...
            (; substrate, vmax, load = ls, pv = ps, status = "SOLVED", objective = obj,
                maxgap = s.maxgap, maxratio = s.maxratio, n_at_vmax = s.n_at_vmax,
                min_branch_P = s.min_P, vpeak = s.vpeak,
                class = s.maxratio <= 1.0 ? "exact" : "inexact", fail_reason = "")
        catch err
            msg = sprint(showerror, err)
            (; substrate, vmax, load = ls, pv = ps,
                status = "FAILED:" * string(nameof(typeof(err))), objective = NaN,
                maxgap = NaN, maxratio = NaN, n_at_vmax = -1, min_branch_P = NaN, vpeak = NaN,
                class = occursin("INFEASIBLE", msg) ? "infeasible" :
                        occursin("complementarity", msg) ? "guard" : "solver",
                fail_reason = replace(first(msg, 200), '\n' => " | "))
        end
        push!(rows, row)
        @printf("[%3d/%3d] %-8s vmax=%-6s load=%-5s pv=%-4s -> %-11s ratio=%-11s atVmax=%-4s vpeak=%s\n",
            i, total, substrate, vmax, ls, ps, row.class,
            isfinite(row.maxratio) ? string(round(row.maxratio; sigdigits = 4)) : "—",
            row.n_at_vmax, isfinite(row.vpeak) ? string(round(row.vpeak; sigdigits = 5)) : "—")
        flush(stdout)
    end
...
function tol_ladder(substrate::Symbol, points)
    ...
    for (vmax, ls, ps) in points
        ...
        for tol in (nothing, 1e-10)
            opt = tol === nothing ? base :
                  optimizer_with_attributes(base.optimizer_constructor, base.params...,
                "tol_gap_abs" => tol, "tol_gap_rel" => tol)
            df = try
                feeder, aggs = substrate === :highpv ?
                               (highpv_feeder(; vmax),
                    [highpv_house(b; pv_scale = ps, load_scale = ls) for b in 2:3]) :
                               (with_vmax(ieee123_modified(), vmax),
                    [ieee123_house(b; pv_scale = IEEE123_BASE.pv * ps,
                        load_scale = IEEE123_BASE.load * ls, dev_scale = IEEE123_BASE.dev)
                     for b in ieee123_load_nodes()])
                ctx, obj, _ = solve_welfare(feeder, ConvexBranchFlow(), aggs;
                    T = T, λ₀ = PRICE, optimizer = opt, allow_export = true, rtol_exact = 1e6)
                s = cone_stats(ctx, feeder)
                @printf("  tol_gap=%-7s ratio=%-12s maxgap=%-11s obj=%.6f  %s\n",
                    tol === nothing ? "1e-8*" : string(tol),
                    round(s.maxratio; sigdigits = 5), round(s.maxgap; sigdigits = 4), obj,
                    s.maxratio <= 1 ? "EXACT" : "inexact")
            catch e
                @printf("  tol_gap=%-7s FAILED: %s\n", tol === nothing ? "1e-8*" : string(tol),
...
function report(df::DataFrame, label::AbstractString, path::AbstractString)
    open(path, "w") do io
        ...
        println(io, "Detector: cone residual |l·v − (P²+Q²)| vs atol=1e-6 + rtol=1e-4·magnitude")
        println(io, "NO AC/Ipopt oracle involved. rtol_exact neutralized so inexact solves are")
        ...
        if !isempty(s)
            @printf(io, "\nratio range (solved): %.4g .. %.4g\n",
                minimum(s.maxratio), maximum(s.maxratio))
            @printf(io, "vpeak range:          %.5f .. %.5f\n",
                minimum(s.vpeak), maximum(s.vpeak))
            ...
            if !isempty(ex) && !isempty(inx)
                @printf(io, "\nmax ratio among EXACT:   %.4g\nmin ratio among INEXACT: %.4g\n",
                    maximum(ex.maxratio), minimum(inx.maxratio))
                println(io, "(a wide empty band between these two means the boundary is")
                println(io, " insensitive to where the threshold is placed; a narrow one means")
                println(io, " the classification is threshold-dependent — see the tolerance ladder)")
            end
        end
        println(io, "\nCAVEAT: atol=1e-6 sits at Clarabel's achievable cone residual on a large")
        println(io, "feeder at the default tol_gap=1e-8. A ratio near 1 is NOT evidence — verify")
...
function main(args)
    ...
    if which in ("both", "highpv")
        ...
        report(df, "3-bus high-PV stress fixture (r=x=0.05, vmin=0.95)",
        ...
        ctl(ps, ls, vm) = only(filter(r -> r.pv == ps && r.load == ls && r.vmax == vm, eachrow(df)))
    ...
    if which in ("both", "ieee123")
        ...
        report(df, "real IEEE-123 OpenDSS positive-sequence impedances, Phase-17-retuned population",
