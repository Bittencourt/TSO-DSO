...
function parse_terminal(tok::AbstractString)
    ...
    m === nothing && throw(
        ArgumentError(
            "could not parse a leading terminal number out of bus token \"$tok\"",
        ),
    )
...
function parse_line_records(master_text::AbstractString)
    line_re =
        r"New\s+Line\.(\S+)\s+.*?Bus1=(\S+?)(?:\.\S+)?\s+Bus2=(\S+?)(?:\.\S+)?\s+LineCode=(\d+)\s+Length=([\d.]+)"i
    records = LineRecord[]
...
function parse_linecode_rx(linecodes_text::AbstractString, n::Integer)
    ...
    m === nothing && throw(
        ArgumentError(
            "could not find an rmatrix/xmatrix block for linecode.$(n) in $(LINECODES_DSS)",
        ),
    )
...
function parse_edges(ieee123_text::AbstractString)
    ...
    m === nothing && throw(
        ArgumentError("could not find `const IEEE123_EDGES = [...]` in $(IEEE123_JL)"),
    )
    tup_re = r"\((\d+),\s*(\d+)\)"
    return Tuple{Int, Int}[
        (parse(Int, tm.captures[1]), parse(Int, tm.captures[2])) for
        tm in eachmatch(tup_re, m.captures[1])
    ]
end
...
function parse_switch_edges(ieee123_text::AbstractString)
    m = match(
        Regex("const\\s+IEEE123_SWITCH_EDGES\\s*=\\s*Set\\(\\[(.*?)\\]\\)", "s"),
        ieee123_text,
    )
    m === nothing && throw(
        ArgumentError(
            "could not find `const IEEE123_SWITCH_EDGES = Set([...])` in $(IEEE123_JL)",
        ),
    )
    tup_re = r"\((\d+),\s*(\d+)\)"
...
function find_line_record(records::Vector{LineRecord}, p::Int, c::Int)
    ...
    throw(
        ArgumentError(
            "no New Line.* record found for edge ($p, $c) in either bus order — check $(MASTER_DSS)",
        ),
    )
end
...
function emit_output(
    ...
    for key in sort!(collect(keys(rx)))
        ...
        println(
            io,
            "    ($(key[1]), $(key[2])) => ($(r_Ω), $(x_Ω)),   # LineCode=$(lc), Length=$(len)",
        )
    end
    println(io, ")")
...
function verify()
    ...
    length(linecodes) == 12 || throw(
        ArgumentError("expected exactly 12 parsed linecodes, got $(length(linecodes))"),
    )
...
function main()
    ...
    println(
        "Wrote $(length(rx)) branch impedances (of $(length(edges)) total edges, " *
