...
**Domain:** Research-grade mathematical-optimization framework for power-system / energy-market problems (Julia + JuMP): convex SOCP branch-flow, ADMM decomposition, and an extensible path to Stackelberg-Nash planning (Benders + diagonalization + MILP).
**Researched:** 2026-07-18
**Confidence:** HIGH (core modeling/solvers verified against the live Julia General registry on 2026-07-18; ecosystem-fit judgments MEDIUM-HIGH)
...
Pkg.add(["JuMP", "MathOptInterface", "Clarabel", "HiGHS", "Ipopt"])
...
Pkg.add(["CSV", "DataFrames", "CairoMakie", "DrWatson"])

# Dev/test/docs
Pkg.add(["TestItemRunner", "Aqua", "JET", "BenchmarkTools"])
Pkg.add(["Documenter", "Literate", "JuliaFormatter"])
```

Commit both `Project.toml` **and** `Manifest.toml` (a research project, not a library, so pin the full environment). Set `[compat]` lower bounds for every direct dep. DrWatson's `tagsave` will additionally embed the active commit/Manifest into every results file.
...
Both can express the SOCP branch-flow model. Choose **JuMP** because this is a framework, not a one-off model:
...
Use Convex.jl only for a throwaway sanity model if you want to double-check a small SOCP by an independent DCP path. Otherwise, JuMP throughout.

### 2. Conic solver selection — **Clarabel primary** (HIGH confidence)

Ranking for SOCP branch-flow **with quadratic objectives and price (dual) recovery**:

1. **Clarabel.jl** — native-Julia IPM, direct quadratic-objective support, high-accuracy primal+dual, actively developed. Best default. (Mosek is its commercial peer; use if licensed.)
2. **SCS** — first-order operator-splitting (ADMM internally). Scales to very large cones but lower-accuracy duals and no native quadratic objective (bridged to a cone). Fine for scouting scale; **not** for certifying SOCP exactness or reporting DADPs.
3. **ECOS** — older SOCP+exp IPM, effectively superseded by Clarabel, no native quadratic. **Do not adopt** for new work.

Idiomatic abstraction in JuMP: a single factory, e.g.

```julia
make_solver(:conic) = optimizer_with_attributes(Clarabel.Optimizer, "verbose" => false)
make_solver(:milp)  = optimizer_with_attributes(HiGHS.Optimizer)
make_solver(:nlp)   = optimizer_with_attributes(Ipopt.Optimizer)
# swap Clarabel→SCS/Mosek/Gurobi by changing one line; models never name a solver.
```

Models receive `Model(make_solver(:conic))` (or `set_optimizer`). This *is* the required solver abstraction — no bespoke wrapper layer needed; lean on JuMP/MOI. Because MOI bridges normalize the quadratic objective and cones, the same model text runs on every backend, so cross-solver validation (Clarabel vs Ipopt vs Mosek) is nearly free — use it to certify exactness.

### 3. PowerModels(Distribution) vs from-scratch — **from-scratch, with PMD as oracle** (MEDIUM-HIGH confidence)

PowerModels.jl (transmission) and PowerModelsDistribution.jl (distribution) are excellent, mature (PMD 0.16.0, PM 0.21.6, on InfrastructureModels 0.7.9), and *do* ship branch-flow relaxations (`SOCBFPowerModel`, and PMD's `LPUBFDiagPowerModel`/LinDist3Flow). But building the operational **core** on them fights this project:
...
**But do not ignore them.** Use PMD/PM as:
...
Skip **PowerModelsONM** (4.0.0) entirely — it targets unbalanced networked-microgrid restoration, orthogonal to this work.

### 4. Bilevel / equilibrium tooling (planning layer) — **hand-rolled Benders + diagonalization** (MEDIUM confidence)

The thesis planning method is explicit: Benders (Stackelberg), Gauss-Seidel diagonalization (Nash), binary-expansion + Lagrangian/integer-L-shaped cuts (MILP). That is an **algorithm to implement**, not a solver to call. Recommendation:
...
There is no dominant, must-use decomposition framework for this pattern; the idiomatic Julia research approach is a hand-written outer loop over JuMP subproblems — which the thesis itself prescribes ("native outer ADMM loop").
...
**If v1 operational layer (centralized GLB-CVX):**
...
**If MPC / rolling-horizon / real-time pricing milestone:**
...
**If planning layer (Stackelberg-Nash):**
...
---
*Stack research for: Julia (JuMP) research-grade power-system optimization framework — TSO-DSO transactive-energy / Stackelberg-Nash*
*Researched: 2026-07-18*
