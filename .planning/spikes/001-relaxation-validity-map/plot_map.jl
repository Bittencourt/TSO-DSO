...
cell(vmax, ps, ld) = only(filter(r -> r.vmax == vmax && r.pv_scale == ps && r.load_scale == ld,
    eachrow(df)))

fig = Figure(size = (1180, 720), backgroundcolor = :white)

Label(fig[0, 1:3], "Where the SOC branch-flow relaxation is EXACT",
...
Label(fig[1, 1:3],
    ...
    fontsize = 12.5, color = :gray35, padding = (0, 0, 0, 2))
...
for (k, vmax) in enumerate(vms)
    ax = Axis(fig[2, k],
        title = "vmax = $vmax",
        titlesize = 15,
        xlabel = "pv_scale", ylabel = k == 1 ? "load_scale" : "",
    ...
    for (i, ps) in enumerate(pvs), (j, ld) in enumerate(lds)
        r = cell(vmax, ps, ld)
        r.n_at_vmax >= 1 && scatter!(ax, [i], [j], marker = :circle, markersize = 5,
            color = (:white, 0.85), strokecolor = :black, strokewidth = 0.6)
    end
    ...
    if vmax == 1.05
        ...
        scatter!(ax, [ie, ii], [je, ji], marker = :star5, markersize = 17,
            color = :white, strokecolor = :black, strokewidth = 1.3)
    end
...
for (k, vmax) in enumerate(vms)
    ...
    hm = heatmap!(ax, 1:length(pvs), 1:length(lds), Z,
        colormap = :vik, colorrange = (lo, hi), nan_color = RGBf(0.94, 0.94, 0.95))
    # The classification threshold sits at ratio = 1, i.e. log10 = 0.
    contour!(ax, 1:length(pvs), 1:length(lds), Z, levels = [0.0],
...
labels = ["exact (ratio ≤ 1)", "INEXACT (ratio > 1)", "guard tripped — UNMEASURED",
    "infeasible — unserveable", "voltage bound active", "EXACT-04 control point"]
Legend(fig[4, 1:3], elems, labels, orientation = :horizontal, framevisible = false,
    labelsize = 12, nbanks = 1, padding = (0, 0, 0, 0))
