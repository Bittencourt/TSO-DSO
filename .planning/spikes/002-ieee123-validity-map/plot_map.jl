...
const CODE = Dict("exact" => 1, "inexact" => 2, "guard" => 3, "solver" => 3, "infeasible" => 4)
const COLORS = [
...
Label(fig[0, 1:3], "REAL IEEE-123 impedances — NO structural inexactness found",
    fontsize = 21, font = :bold, padding = (0, 0, 4, 0))
Label(fig[1, 1:3],
    ...
    fontsize = 12, color = :gray35, padding = (0, 0, 0, 2))

# ── Row 1: categorical ────────────────────────────────────────────────────────────────────
for (k, vmax) in enumerate(vms)
    ax = Axis(fig[2, k], title = "vmax = $vmax", titlesize = 15,
        xlabel = "pv multiplier", ylabel = k == 1 ? "load multiplier" : "",
        xticks = (1:length(pvs), string.(pvs)), yticks = (1:length(lds), string.(lds)),
        xticklabelsize = 11, yticklabelsize = 11, xgridvisible = false, ygridvisible = false)

    Z = [begin
             r = cell(vmax, pm, lm)
             r === nothing ? NaN : Float64(CODE[r.class])
         end for pm in pvs, lm in lds]
    heatmap!(ax, 1:length(pvs), 1:length(lds), Z,
    ...
    for (i, pm) in enumerate(pvs), (j, lm) in enumerate(lds)
        r = cell(vmax, pm, lm)
        r !== nothing && r.n_at_vmax >= 1 && scatter!(ax, [i], [j], marker = :circle,
            markersize = 5, color = (:white, 0.85), strokecolor = :black, strokewidth = 0.6)
    end

    # The thesis-reproduction anchor lives at (pv×1.0, load×1.0) on the vmax = 1.10 panel.
    if vmax == 1.10
        ia, ja = findfirst(==(1.0), pvs), findfirst(==(1.0), lds)
        scatter!(ax, [ia], [ja], marker = :star5, markersize = 18, color = :white,
            strokecolor = :black, strokewidth = 1.3)
    end
...
for (k, vmax) in enumerate(vms)
    ax = Axis(fig[3, k], xlabel = "pv multiplier",
    ...
    hm = heatmap!(ax, 1:length(pvs), 1:length(lds), Z, colormap = :vik,
        colorrange = (lo, hi), nan_color = RGBf(0.94, 0.94, 0.95))
    contour!(ax, 1:length(pvs), 1:length(lds), Z, levels = [0.0], color = :black,
        linewidth = 2.2)
    k == length(vms) && Colorbar(fig[3, length(vms) + 1], hm,
        label = "log₁₀ cone-gap ratio", labelsize = 12, ticklabelsize = 11)
    xlims!(ax, 0.5, length(pvs) + 0.5)
    ylims!(ax, 0.5, length(lds) + 0.5)
end
...
elems = [
    ...
    PolyElement(color = COLORS[3]), PolyElement(color = COLORS[4]),
    MarkerElement(marker = :circle, markersize = 6, color = (:white, 0.85),
        strokecolor = :black, strokewidth = 0.6),
    MarkerElement(marker = :star5, markersize = 13, color = :white,
        strokecolor = :black, strokewidth = 1.2),
]
labels = ["ratio ≤ 1", "flagged ratio > 1 — SOLVER NOISE", "guard tripped — UNMEASURED",
    "not swept", "voltage bound active (never occurs)", "thesis-reproduction point"]
Legend(fig[4, 1:3], elems, labels, orientation = :horizontal, framevisible = false,
...
Label(fig[5, 1:3],
    ...
    fontsize = 11, color = :gray40, halign = :left, justification = :left)
