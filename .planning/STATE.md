...
- [CI flaky — Julia 1.11 IEEE-13 ADMM `NUMERICAL_ERROR`] (2026-07-22): the `test`-matrix Julia **1.11** leg intermittently fails the IEEE-13 ADMM cross-validation + acceptance testitems with Clarabel `termination_status = NUMERICAL_ERROR` (caught correctly by `assert_solved!` in `src/core/status.jl` — the gate refuses to trust the result). Julia **1.10 and 1.12 always pass**, and 1.11 **passed on the very next run** (commit 61c60a4), so this is INTERMITTENT, not a hard break. Root cause is the documented cone-slack numerical sensitivity of that IEEE-13 ADMM solve (per-unit-base dependent — see the IEEE-123 impedance note above). NOT fixed — flagged for a deliberate numerical-robustness pass (candidate levers: Clarabel tolerance/`equilibrate` settings, per-unit base, a bounded solve-retry in the ADMM subproblem). Does not block the docs deploy or the milestone; 1937+/1941 pass on the affected run.
