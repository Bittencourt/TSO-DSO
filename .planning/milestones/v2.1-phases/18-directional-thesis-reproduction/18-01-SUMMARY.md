...
provides:
  ...
  - "The empirical fact that the DSO-surplus sign flip (FIT dso<0 -> DADP dso>0) is confirmed at the exact Phase-17-retuned point but does NOT survive ±2%/±5% population-scale perturbation in EITHER direction (all 4 non-zero sweep points fail the SOCP-exactness gate outright)"
affects: [18-02, 18-03]
...
key-decisions:
  ...
  - "sign_flip_survives=false is reported honestly — the sweep was NOT narrowed and no failing point was omitted to force a passing appearance"
...
- **Verification:** Re-ran the full script after the fix; it completed end-to-end and produced the committed `results/repro_stability_check/findings.txt` with all 4 non-zero sweep points correctly reported as `FAILED` rows plus their exact error messages, and `sign_flip_survives: false` reported honestly.
