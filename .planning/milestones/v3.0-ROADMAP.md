# v3.0 Research Extension Rungs — Milestone Archive

**Status:** Shipped (gaps accepted)
**Phases:** 19–25 (7 phases, 41 planned plans + 2 unplanned gap-closure waves)
**Started:** 2026-07-26
**Shipped:** 2026-08-24
**Audit:** [`v3.0-MILESTONE-AUDIT.md`](v3.0-MILESTONE-AUDIT.md) — `gaps_accepted`
**Git range:** `v2.1.1..v3.0` (453 commits, 291 files, +84,308 / -504)

## Milestone Summary

Seven research-extension axes, each shipping as a documented "rung" with its own certificate:
4Q-BESS + live reactive dual-ascent; an overvoltage-capable restricted relaxation; closed-loop
MPC/rolling-horizon real-time pricing; a two-stage stochastic extensive form; meshed (non-radial)
networks with an angle-recoverability certificate; discrete/integer investment expansion via
Laporte–Louveaux cuts; and a real-utility-scale IEEE-8500 benchmark that honestly characterized a
memory wall rather than claiming a result it never measured.

### Key Accomplishments

1. **Phase 19 — MESH-04/05:** `FourQuadBESS` with genuine P/Q variables inside an apparent-power
   cone, and the v2.1 reactive-dual scaffolding promoted to a live, converging μ-ascent, gated by a
   *measured* (not assumed) `:live` cross-validation plus a liveness regression proving the mechanism
   actually reacts to its input.
2. **Phase 20 — OVR-01..04:** A restricted SOCP that prices the v2.1 EXACT-04 high-PV overvoltage
   regime, carrying its own AC-certified validity certificate. Notably includes an **honest negative
   result** (the OPF-ε variant) alongside the OPF-m success (`socp_maxgap = 2.08e-8`), and
   establishes the restriction-certificate pattern Phase 23 later reuses.
3. **Phase 21 — MPC-01..04:** Closed-loop receding-horizon solves over stateful devices publishing
   rolling DADPs as an RTP signal, benchmarked against perfect foresight — and the milestone's
   standout genuine cross-phase wire: it reuses Phase 20's escalation ladder rather than
   reimplementing it.
4. **Phase 22 — STOCH-01..04:** Two-stage extensive-form welfare solve over seeded Markov scenarios
   with per-scenario DADPs as the primary price output, and a never-aggregated per-scenario SOCP
   exactness gate.
5. **Phase 23 — MESH-01/02/03/06:** A parallel `MeshedFeeder` + non-radial SOCP formulation whose
   angle-recoverability certificate exercises BOTH verdicts (recoverable / structurally
   unrecoverable) on one committed 4-bus diamond fixture.
6. **Phase 24 — INT-01..04:** Binary-expansion integer investment with genuine Laporte–Louveaux
   integer optimality cuts, certified against exhaustive lattice enumeration, with the PVAL-04
   no-binaries guard **scoped rather than deleted** via a self-verifying per-builder exemption.
7. **Phase 25 — SCALE-01..05:** The public IEEE-8500 balanced load case (full MV + LV secondary,
   ~4.9k buses after positive-sequence collapse) as a committed, provenance-tracked fixture, plus a
   density-sweep benchmark that **reported a memory wall honestly** instead of substituting the
   MV-only control fixture's numbers for the headline point.

### Methodological Lesson Worth Carrying Forward

Phase 24's **per-cut-validity certificate** — a *mechanism* test rather than an *outcome* test —
caught an invalid Laporte–Louveaux cut that an exhaustive 256-pair algebra proof had passed cleanly.
The algebra proof validated the cut formula against a *fixed* recourse input, so it structurally
could not see a defect in what supplied that input. Three stacked defects sat underneath (recourse
at the wrong point; an anti-stall heuristic banning the optimal corner; HiGHS's default
`mip_feasibility_tolerance` leaving a ~7e-8 residual), and code review then found two more. Test the
mechanism, not just the answer.

A second, cheaper lesson: an outer criterion advertising itself as **exact** silently inherits inner
solver slack unless `mip_rel_gap` AND `mip_feasibility_tolerance` are both set explicitly.

### Accepted Gap

**SCALE-05** — the true ~40x headline IEEE-8500 fixture was OOM-killed at every density, so solve
time / ADMM iteration count / SOCP exactness at headline scale come only from the 2,521-bus MV-only
*control* fixture (~20x). Accepted by the researcher as an honest non-measurement. Kills occurred at
6.8–9.75 GiB anon-rss on a shared 15 GiB machine with ~9 GiB of unrelated swap pressure, so the true
memory requirement is **lower-bounded only**. Optimization work is carved out as SCALE-STRETCH.
