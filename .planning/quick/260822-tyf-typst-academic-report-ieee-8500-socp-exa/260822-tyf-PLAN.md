---
quick_id: 260822-tyf
description: Author docs/writeups/ieee8500_exatidao_socp.typ — a pt-BR academic Typst report documenting the IEEE-8500 SOCP-exactness investigation — and register it in docs/writeups/README.md
date: 2026-08-22
mode: quick
---

# Quick Task 260822-tyf: IEEE-8500 SOCP-exactness investigation writeup

## Why

The IEEE-8500 SOCP-exactness investigation (deferred-items.md Items 1/3/5, three quick tasks
`260822-oi7`/`260822-pxb`/`260822-rle`, and the calibration/gap-report CSVs) is currently scattered
across `deferred-items.md`, `25-DATA-PROVENANCE.md`, three quick-task `SUMMARY.md` files, and two
committed CSVs. It has real thesis-appendix value — a genuinely auditable investigation including
three wrong turns — but no single reader-facing document. This task consolidates it into one Typst
report matching the project's two existing writeups' house style, in Portuguese, with every wrong
turn explicitly labelled and every number traceable to an already-measured source.

**Hard constraint carried from the planning prompt: do not invent any number.** Every figure in
Task 1's content below is copied verbatim from `deferred-items.md`, `25-DATA-PROVENANCE.md`, the
three quick-task summaries, or the committed CSVs — the executor transcribes, does not re-derive or
round differently.

**One resolved inconsistency, flagged so the executor does not "fix" it back:** the planning
prompt's Stage-B narrative says the fabricated-impedance override was "superseded in Stage F," but
Stage H's own text ("Pass 1 merged 3 pairs — the two 1-ft splits plus the connector, whose Stage-B
override was REVERTED") is the actual superseding event per `deferred-items.md`/`25-DATA-PROVENANCE.md`
(quick task `260822-pxb`). Task 1's content below says "Superada no Estágio H" — write it exactly as
given in Task 1, not "Estágio F".

## Context

- `docs/writeups/stackelberg_vs_psr_n1n2.typ` and `docs/writeups/thesis_caseA.typ` — the two house-style
  references. Preamble is byte-identical between them (see Task 1's embedded copy below); title block
  is `#align(center)[...]` with 20pt bold title, `v(0.3em)`, 12pt gray subtitle, `v(0.8em)`,
  `#line(length: 60%, stroke: 0.5pt + gray)`, `v(0.8em)`; then an ITALIC provenance paragraph; then
  `#v(0.3em)`; then `= Resumo`. References footer: `#v(1em)`, `#line(length: 100%, stroke: 0.4pt +
  gray)`, `#v(0.3em)`, `#text(size: 8.5pt, fill: gray)[*Referências:* \ ...]` with `\` line breaks.
- `.planning/quick/260728-co0-.../260728-co0-SUMMARY.md` — records two real compile-failure gotchas
  from prior authoring of this exact house style, BOTH must be honoured here:
  1. Never use `auto` for a long-text table column — a wide `auto` column collapses `fr` columns to
     zero width and scrambles rendered text (silent — still compiles cleanly). Every table in Task 1
     below already specifies explicit `fr` fractions for every column; do not substitute `auto`.
  2. Never write a bare multi-letter identifier inside `$...$` math mode (`$N1$` was rejected).
     Task 1's content below keeps all multi-letter identifiers (`atol`, `rtol`, `r_pu`, bus/line
     names, CLI flags) as backtick code spans OUTSIDE math mode; only single-letter physics symbols
     (`l`, `v`, `P`, `Q`) appear inside `$...$`. Preserve this split exactly.
- `.planning/phases/25-ieee-8500-scalability-benchmark/deferred-items.md` and `25-DATA-PROVENANCE.md`
  — the authoritative finding record; every number in Task 1 traces to one of these two files, or to
  `results/ieee8500_benchmark/socp_gap_report.csv` / `noise_floor_calibration.csv`, or to
  `src/models/exactness.jl`'s docstring/error message (verified verbatim: the throw message ends
  `"prices REFUSED (thesis 3.43-3.45; PF-04)"`, and `assert_socp_exact!` uses the combined bound
  `gap <= atol + rtol · max(|lhs|, |rhs|)`).
- `docs/writeups/README.md` — table has 3 columns (`Writeup`, `Companion script`, `Figures`). The
  `stackelberg_vs_psr_n1n2.typ` row uses `—` for both non-writeup columns (no companion script, no
  generated figures) — this report follows the SAME pattern (it consolidates existing CSVs/docs, it
  does not run a script or generate figures).
- `.gitignore` line 43 (`/docs/writeups/*.pdf`) already covers any compiled PDF from this task — no
  `.gitignore` edit needed. `typst 0.15.1` is on `PATH` at `/home/pedro/.local/bin/typst`.
- No figures are embedded — per the planning prompt's explicit constraint, this investigation
  produced CSV evidence, not plots, and no committed figure under `results/` documents this specific
  investigation.

## Tasks

### Task 1 — author `docs/writeups/ieee8500_exatidao_socp.typ`

- **files:** `docs/writeups/ieee8500_exatidao_socp.typ` (new)
- **action:**

  Write the file below verbatim via the Write tool (not a heredoc). Every number is sourced from the
  planning prompt's `<content_the_report_must_carry>` block (itself verified against
  `deferred-items.md`/`25-DATA-PROVENANCE.md`/the three quick-task summaries/the two CSVs during
  planning) — do not alter any figure, do not add commentary numbers not present here, do not omit
  any of the three labelled wrong turns.
...
- **verify:** File exists at `docs/writeups/ieee8500_exatidao_socp.typ`; `grep -c "VOLTA ERRADA" docs/writeups/ieee8500_exatidao_socp.typ` returns 3; `grep -cE "\<atol\>" docs/writeups/ieee8500_exatidao_socp.typ` (word-boundary, avoids matching inside longer identifiers) returns > 0; `grep -n "columns:" docs/writeups/ieee8500_exatidao_socp.typ | grep -c auto` returns 0 (checks only table `columns:` lines for `auto` — a bare substring grep on the whole file would false-positive on words like "tautológica" or "auto-laços").
- **done:** The `.typ` file exists with all nine sections (Resumo through Referências), all three wrong turns labelled `VOLTA ERRADA 1/2/3`, and every number matches this task's embedded content verbatim.

### Task 2 — register in `docs/writeups/README.md`

- **files:** `docs/writeups/README.md`
- **action:** Add one row to the existing table, matching the `stackelberg_vs_psr_n1n2.typ` row's
  style exactly (pipe-separated, `—` for the two non-writeup columns since this report has no
  companion script and embeds no figures):
  ...
  Insert it as the new last row of the table (after the `stackelberg_vs_psr_n1n2.typ` row). Do not
  reformat any existing row.
- **verify:** `grep -c "ieee8500_exatidao_socp" docs/writeups/README.md` returns 1; the table still
  has exactly one header separator row (`grep -c '^|---'` or equivalent — visually confirm no
  duplicate header was introduced).
- **done:** `README.md`'s table has a new row for the new writeup, format-consistent with the
  existing three rows.

### Task 3 — compile, verify rendering, verify git hygiene, spot-check numbers

- **files:** none (verification only)
- **action:**
  1. Compile: `/home/pedro/.local/bin/typst compile docs/writeups/ieee8500_exatidao_socp.typ` — must
     exit 0 and produce `docs/writeups/ieee8500_exatidao_socp.pdf`.
  2. **Real check for the silent scrambled-table failure mode** (a clean exit is NOT sufficient
     evidence — the `auto`-column gotcha compiles cleanly while still corrupting text): render every
     page to PNG into the scratchpad directory, e.g.
     `/home/pedro/.local/bin/typst compile --format png docs/writeups/ieee8500_exatidao_socp.typ "$SCRATCHPAD/page-{p}.png"`
     (substitute the actual scratchpad path), then use the Read tool on each page image and visually
     confirm: (a) the four tables (Estágio F, Estágio H's 8-segment table, the 3-pass result table)
     render with readable, non-overlapping cell text and properly proportioned columns; (b) no text
     runs off the page or collides with the header/footer.
  3. Git hygiene: `git status --porcelain docs/writeups/` must show ONLY
     `docs/writeups/ieee8500_exatidao_socp.typ` (untracked/new) and `docs/writeups/README.md`
     (modified) — the compiled `.pdf` must NOT appear (confirms `.gitignore:43` still covers it).
  4. Spot-check at least 6 numbers against their committed sources, e.g.: `grep "0.0325\|0.0018221\|0.0325016" results/ieee8500_benchmark/socp_gap_report.csv results/ieee8500_benchmark/noise_floor_calibration.csv`
     to confirm the headline `ieee8500 @ 0.1, T=10` pre-merge/pass-2 gaps (`3.250e-2`, `1.822e-3` in
     the doc) match; similarly confirm `0.0011460286`/`0.0049691451` (the two `atol` values) appear
     verbatim in `scripts/benchmark_ieee8500.jl` or the cited CSVs/summaries; confirm
     `3b208397160213cae4a9e2d0a7d1aa3528ce26e1` matches `25-DATA-PROVENANCE.md`'s pinned commit SHA.
- **verify:**
  - `typst compile docs/writeups/ieee8500_exatidao_socp.typ; echo $?` prints `0`.
  - All rendered PNG pages visually confirmed clean (no scrambled/overlapping table text) via Read.
  - `git status --porcelain docs/writeups/` shows exactly 2 lines (the new `.typ`, the modified
    `README.md`), no `.pdf` line.
  - All spot-checked numbers reproduce their source verbatim (report back any mismatch found — do
    not silently "fix" the `.typ` to a different number without re-confirming against source first).
- **done:** Clean compile, visually-verified table rendering, correct git hygiene, and every
  spot-checked number confirmed to trace to a committed source.

## Success Criteria

- `docs/writeups/ieee8500_exatidao_socp.typ` exists, compiles with `typst compile` exiting 0, and is
  registered in `docs/writeups/README.md`.
- All three wrong turns (Estágio B, C, E) appear, explicitly labelled `VOLTA ERRADA 1/2/3`.
- No table renders with scrambled/overlapping text (verified visually via rendered PNG pages, not
  inferred from a clean compile exit code alone).
- `git status --porcelain docs/writeups/` shows only the new `.typ` and the modified `README.md` —
  the compiled `.pdf` stays gitignored.
- Every number in the document traces verbatim to `deferred-items.md`, `25-DATA-PROVENANCE.md`, one
  of the three quick-task `SUMMARY.md` files, `socp_gap_report.csv`, `noise_floor_calibration.csv`,
  or `src/models/exactness.jl` — none invented, none re-derived by re-running a solve.
