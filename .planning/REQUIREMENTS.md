...
- [ ] **INFRA-01**: Researcher can instantiate the Julia package with a pinned environment
      (`Project.toml` + committed `Manifest.toml`, `[compat]` floors) that resolves cleanly on Julia 1.10 LTS + 1.11.
- [ ] **INFRA-02**: Any model requests its solver via a single `select_optimizer(::ProblemClass)`
...
- [ ] **DATA-01**: Researcher can define a radial distribution feeder (buses, branches with r/x, limits,
      MEM frontier node) as immutable data structs independent of JuMP.
- [ ] **DATA-02**: The framework validates that a feeder is radial (tree) and reports a clear error
      otherwise (SOC exactness holds only on radial trees).
- [ ] **DATA-03**: Modified IEEE 13-node and 123-node test feeders ship as built-in fixtures with the
      thesis parameters (voltage limits, head-branch capacity, price profiles).
- [ ] **DATA-04**: Researcher can generate seeded inelastic-demand and PV profiles via first-order
      Markov-chain synthesis (data generation only, outside the optimization).

### Prosumer Device Library

- [ ] **DEV-01**: Thermostatic-load model (temperature linear in power, comfort bounds) contributing
      variables, constraints, and a quadratic utility term to a model.
- [ ] **DEV-02**: Programmable/deferrable-load model (energy-within-window) with quadratic utility.
- [ ] **DEV-03**: Interruptible/elastic-load model (power bounds) with concave quadratic utility.
- [ ] **DEV-04**: PV + battery (BESS) model with SOC dynamics, PV-limited charge, and charge-utility /
      discharge-cost preferences — **without binary variables** (App. C parametrization guarantees no simultaneous charge/discharge).
- [ ] **DEV-05**: Aggregator aggregates its prosumers' devices into nodal net active/reactive power and
      total utility; device modules never reference the network directly.

### Power-Flow Formulations

- [ ] **PF-01**: A swappable power-flow interface (`AbstractPowerFlow`) lets a formulation contribute
      branch/voltage terms into the shared nodal-balance residual with no `if formulation ==` branching.
- [ ] **PF-02**: DC / LinDistFlow linear formulations for the lower rungs of the abstraction ladder.
- [ ] **PF-03**: SOCP Convex Branch Flow formulation (DistFlow SOC relaxation) with the LinDistFlow
      exactness copy (aux `v̂` + affine voltage bounds) as part of the model definition.
- [ ] **PF-04**: After any SOCP solve, an automated invariant asserts relaxation exactness
      (`max|l·v − (P²+Q²)| < τ` per branch) on both an easy and a high-PV/over-voltage fixture.

### Objective & Centralized Solve

- [ ] **OPT-01**: Social-welfare objective `GLB-CVX` = Σ aggregator utility − wholesale (MEM) purchase
      cost, assembled from device utility terms + the power-flow model.
- [ ] **OPT-02**: A centralized monolithic solve produces the global optimum as ground truth, with the
      nodal-balance dual available.
- [ ] **OPT-03**: The centralized solve is exposed as an `operational_oracle(z) → (cost, π)` returning
      the frontier coupling dual, so the future planning game wraps it without a rewrite.

### Pricing & Welfare Accounting

- [ ] **PRICE-01**: Extract the day-ahead dynamic price (DADP/DLMP) as the dual of the nodal
      active-power balance, per node per hour.
- [ ] **PRICE-02**: Decompose the DLMP into energy / loss / congestion / voltage components that sum to
      the nodal price with correct sign (validated by assertion).
- [ ] **PRICE-03**: Welfare accounting split into social / DSO / prosumer surplus, plus a FIT baseline
      for comparison (reproduces the +25%-social-welfare headline).
- [ ] **PRICE-04**: Economic-direction sanity checks (price below wholesale at PV glut, above at congestion).

### ADMM Decomposition

- [ ] **ADMM-01**: ADMM decomposition solves the operational problem via per-node aggregator subproblems
      (`AGR-OPT`) + per-hour network subproblem (`DSO-OPT`) with dual ascent, reusing the same builders as centralized.
- [ ] **ADMM-02**: ADMM stops on **both** primal and dual residuals with per-unit-normalized adaptive `ρ`
      (no hard-coded scale-specific penalty).
- [ ] **ADMM-03**: An automated cross-validation test asserts ADMM welfare and duals match the
      centralized optimum on every fixture small enough to solve monolithically.
- [ ] **ADMM-04**: ADMM subproblems are built once and re-solved via parameter/coefficient updates +
      warm starts (no per-iteration model rebuild).
- [ ] **ADMM-05**: Convergence diagnostics (residual traces, iteration count, price convergence) are
...
| PF-01 | Phase 1 | Pending |
| PF-02 | Phase 2 | Pending |
