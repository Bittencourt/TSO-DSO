...
@testitem "dso: build_dso_opt guards — empty aggs, λ₀ shape, transit node, root aggregator" setup = [
    Phase6Fixtures,
    Phase4Fixtures,
] tags = [:dso] begin
    using TSODSO
    ...
    # Transit node: a 3-bus radial feeder with an aggregator only on bus 2 leaves bus 3
    # unclosed — the guard must reject it.
